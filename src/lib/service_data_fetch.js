...
async function postSchedule(scheduleAssignments, plans, preferences) {
...
function removePeopleWithConflicts(
  conflicts,
  ...
  writeJson(teamPositionAssignments, "team_position_assignments_UPDATED.json");

  return teamPositionAssignments;
}

function getPersonWithHighestPriority(people, preferences) {
...
function generateSchedule(neededPositions, availablePeople) {
  ...
  for (let position in neededPositions) {
    ...
    for (let i = 0; i < quanity; i++) {
      let person = getPersonWithHighestPriority(people, preferences);
